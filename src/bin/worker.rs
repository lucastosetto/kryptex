...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let concurrency: usize = env::var("WORKER_CONCURRENCY")
        ...
        .unwrap_or_else(|| {
            symbols.as_ref().map(|s| s.len()).unwrap_or(1).max(1)
        });
    ...
    let symbols = symbols
        .ok_or("SYMBOLS environment variable is required")?;
    ...
    let database = match QuestDatabase::new().await {
        ...
        Err(e) => {
            ...
            warn!("Continuing without database - signals will not be persisted");
